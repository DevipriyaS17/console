...
### Redfish Resource Implementation Matrix

The following table provides a comprehensive overview of how each Redfish resource type is implemented within the DMT Console, including the specific topics and capabilities covered for each resource.

| **Resource Type** | **Category** | **Implementation Scope** | **Key Topics Covered** |
|---|---|---|---|
| **Systems** | Device-Specific | ComputerSystem resource mapping | ComputerSystem schema mapping from AMT data, Processor information extraction and formatting, Memory resource representation, Storage controller and device mapping, Network interface configuration, Power and thermal management data, Boot options and BIOS settings, System actions (Reset, SetBootSource, etc.), Health and status reporting mechanisms |
| **Chassis** | Device-Specific | Physical hardware inventory | Physical chassis information mapping, Thermal sensor data aggregation, Power supply and consumption metrics, Fan control and monitoring, Physical security settings, Asset tracking and inventory data, Environmental monitoring capabilities, Chassis-level actions and controls |
| **Managers** | Device-Specific | Management controller capabilities | Manager schema population from AMT data, Network service configuration, Certificate management and security, Log service implementation, Virtual media capabilities, Remote console access configuration, Firmware update mechanisms, Manager reset and diagnostic actions |
| **AccountService** | Service-Level | User account management | User account management within DMT context, Role-based access control (RBAC) implementation, Password policies and security settings, Account lockout and session management |
| **SessionService** | Service-Level | Session management | Session creation and management, Authentication token handling, Session timeout and cleanup, Concurrent session limitations |
| **EventService** | Service-Level | Event handling | Event subscription management, Event filtering and routing, Push notification mechanisms, Event history and logging |
| **UpdateService** | Service-Level | Firmware coordination | Firmware update coordination across devices, Update scheduling and orchestration, Progress tracking and status reporting, Rollback and recovery mechanisms |
